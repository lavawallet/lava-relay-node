...
module.exports = class LavaPacketSubmitter  {
    ...
    if(relayedPacketsData.pendingCount == 0 )
    {
      ...
      for(var i in lavaPackets)
      {
        ...
        console.log('the signature of this lavapacket is ', signature)
         ...
         var packetIsExecutingBlacklistedContracts = self.packetIsExecutingBlacklistedContracts(packetData)
         ...
         if(  ( !packetWasBroadcastedRecently  ) && !txdata.pending)   // if not, ignore the packet
         {
           ...
           if(packetIsAttractiveStatus.attractive == true && !packetIsExecutingBlacklistedContracts)  //if not , remove from queue
           {
  ...
  async monitorPendingLavaPackets()
  {
    ...
    if(relayedPacketsData.pendingCount >= 1 )    //if a packet is   pending right now ....
    {
