...
module.exports = class LavaPacketSubmitter  {
           ...
           if(packetIsAttractiveStatus.attractive == true)  //if not , remove from queue
           {
                 ...
                 txdata.queued = false;
                  //await this.redisInterface.deleteHashInRedis('queued_tx',signature)
                   ...
                   try{
                     var txHash = await self.broadcastLavaPacket(packetData,broadcastingSpeed,this.web3,this.relayConfig.environment);
                   }catch(err)
                   {
  ...
  static async broadcastLavaPacket(packetData,broadcastingSpeed,web3,env)
  {
