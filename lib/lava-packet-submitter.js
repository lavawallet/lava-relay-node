...
module.exports = class LavaPacketSubmitter  {
  ...
  init() {
    ...
    setTimeout(function(){self.monitorPendingLavaPackets()},0);
  }
    ...
    if(relayedPacketsData.pendingCount == 0 )
    {
      ...
      for(var i in lavaPackets)
      {
        ...
        console.log('the signature of this lavapacket is ', signature)
         ...
         if(  !packetWasBroadcasted && !txdata.pending)   // if not, ignore the packet
         {
           // set it to pending
           if(packetIsAttractive)  //if not , remove from queue
           {
                 ...
                 txdata.queued = false;
                  //await this.redisInterface.deleteHashInRedis('queued_tx',signature)
                   ...
                   try{
                     var txHash = await self.broadcastLavaPacket(packetData);
                   }catch(err)
                   {
                     console.error(err);
                   }
