...
module.exports = class LavaPacketSubmitter  {
                 ...
                 txdata.queued = false;
                  //await this.redisInterface.deleteHashInRedis('queued_tx',signature)
                    ...
                    if( !txResult.success )
                    {
                      ...
                      var txHash = txResult.predictedTxHash  ;
      ...
      for(var i in lavaPackets)
      {
         ...
         if(  packetWasBroadcasted   && !txdata.mined   )
         {
            var packetData = JSON.parse(broadcastedPacketDataJSON)
               console.log('meepo  ', broadcastedPacketDataJSON)
           //get mining receipt
           ...
           }else if(  basePacketData.ethBlockReceived  <  (currentEthBlock - 50 )  )
           {
         ...
         }else  //no broadcast data
         {
           ...
           console.log('inspect base ', basePacketData  , currentEthBlock)
           if(  basePacketData.ethBlockReceived  <  (currentEthBlock - 50 )  )
           {
             console.log('tx wasnt mined in 50 blocks, dropping ')
             txdata.pending=false;
             txdata.errored = true; //check for this and display on dashboard
  ...
  async packetIsAttractiveToSubmit(packetData)
  {
    ...
    return {attractive: (relayRewardAboveSafeLow || relayRewardAboveNormal || relayRewardAboveFast ), fast: relayRewardAboveFast } ;
