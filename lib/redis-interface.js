...
module.exports =  {
    ...
    /*   redisClient.set("string key", "string val", redis.print);
     ...
     async storeRedisData(key, data )
     {
       ...
       {
         console.error('WARN: Tried to store undefined data [key,data] ', key , data )
       }
     ...
     async loadRedisData(key )
     {
      ...
      async peekFromRedisList(key , index )
      {
        return new Promise(function (fulfilled,rejected) {

          redisClient.lindex(key, index, function (err, reply) {
             if(err){rejected(err);return;}
      ...
      async popLastFromRedisList(key )
      {
        ...
        async findHashInRedis( key, hash )
        {
          ...
          if(typeof key == 'undefined' || typeof hash == 'undefined')
          {
            console.error('WARN: Tried to get undefined data [key,hash] ', key , hash )
          }
