  ...
  async sortAndConnectToPeers()
  {
      ...
      for(var i in allPeerKeys)
      {
        ...
        if(!alreadyConnected  && candidateNotSelf
          && connection_candidate.length < TARGET_PEER_CONNECTION_COUNT){
      ...
      shuffle(connectionCandidates);

      //
      for var(i =0;i<additionalConnectionsNeeded;i++)
      {
          connectToPeer( connectionCandidates[i] )
      }
