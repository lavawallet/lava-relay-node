...
import LavaPacketCollector from './lava-packet-collector'
var packetCollector = new LavaPacketCollector();

import LavaPacketSubmitter from './lava-packet-submitter'
var packetSubmitter = new LavaPacketSubmitter();
...
module.exports = class LavaPeerInterface  {
  ...
  */
  ...
  async getTargetSafeLowRewardTokens()
  {

    let _GAS_REQUIRED_FOR_TX_RELAY = 130151;

    var profitMarginPercent =  relayConfig.profitMarginPercent;
    ...
    var ethGasSafeLowGwei = await this.redisInterface.loadRedisData('ethGasSafeLow'  ) ;

    var ethGasSafeLowEth = ethGasSafeLowGwei / (Math.pow(10,9))


    var requiredEthForTXRelay = _GAS_REQUIRED_FOR_TX_RELAY * ethGasSafeLowEth;


    //console.log('requiredEthForTXRelay',requiredEthForTXRelay)
    ...
    //console.log('targetTokensForTXRelay', targetTokensForTXRelay)
  ...
  async getRelayData()
  {
    var relayData = {};
     ...
     relayData.name =  relayConfig.name;
     relayData.profitMarginPercent =  relayConfig.profitMarginPercent;
     ...
     relayData.targetSafeLowRewardTokens = parseFloat( await this.getTargetSafeLowRewardTokens() ).toFixed(3)
