...
module.exports = class LavaPacketCollector {
  ...
  async storeNewLavaPacket(packetData)
  {
    ...
    if( this.lavaPacketHasValidSignature( packetData ) == false )
    {
      return {success:false,message:'packet has invalid signature'}
    }

    if( this.lavaPacketSignatureBurned( packetData ) == true   ) //why do i need == true here ????
    {
  ...
  async lavaPacketSignatureBurned(packetData){
    ...
    var burned = await ContractInterface.getWalletContract(this.web3,'test').signatureBurned.call(packetData.signature)

    console.log('sig was burned?',burned )
